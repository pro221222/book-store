         ...
         using (var db = new book_storeContext())
            {
                ...
                if( user.Password == Pass  && user.Role == 0)
                {
                    string[] users = {user.Id.ToString(), user.Name.ToString(), user.Address.ToString(), user.PhoneNumber.ToString(), user.Role.ToString()};
                    return Ok(users);
                    
                }if(user.Password == Pass  && user.Role == 1)
                {
                    string[] users = {user.Id.ToString(), user.Name.ToString(), user.Address.ToString(), user.PhoneNumber.ToString(), user.Role.ToString()};
                    return Ok(users);
